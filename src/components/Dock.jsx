...
import { Home, Music, Tv, Settings, MessageCircle, Image as ImageIcon, Gamepad2, Monitor, Terminal, Palette, Coins } from 'lucide-react'
...
const apps = [
  ...
  { id: 'reward', label: 'Belly', icon: Coins },
]

const Dock = ({ onOpen, onToggleTheme, onReward }) => {
  return (
    <div className="pointer-events-auto fixed inset-x-0 bottom-6 z-50 flex justify-center">
      ...
      >
        {apps.map(({ id, label, icon: Icon }) => {
          const handleClick = () => {
            if (id === 'theme') return onToggleTheme?.()
            if (id === 'reward') return onReward?.()
            return onOpen?.(id)
          }
          return (
            <button
              key={id}
              onClick={handleClick}
              className="group relative flex h-12 w-12 items-center justify-center rounded-xl bg-white/5 text-white ring-1 ring-white/10 transition hover:-translate-y-1 hover:bg-white/10"
            >
